...
<body>
	...
	<article id="modifyForm">
		...
		<%-- 입력받지 않은 글번호,페이지번호 파라미터를 hdden 속성으로 전달 --%>
								...
								<c:choose>
									<c:when test="${not empty file}">
										...
										<!-- 파일삭제 이미지 생성 -->
											...
											<!-- 하이퍼링크 클릭시 deleteFile() 함수호출 (파라미터: 글번호 .실제파일명,카운트번호) -->
											<!-- 만약 파일명만 표시할때 파일업로드 요소도 파라미터로 전달하려면(파라미터값은 널스트링)
												파일업로드 요소 추가 후 히든 속성으로 숨기면 된다 -->
											<!-- 파일 항목의 name 속성값을 다르게 부여하기 위해 file 문자열과 카운팅 속성값 결합 -->
											<!-- c:foreach 태그의 ststus.count 활용(1부처 1씩 증가) -->
