...
<script src="${pageContext.request.contextPath}/resources/js/jquery-3.7.1.js">
...
<%-- ====================== [ 게시물 검색 기능 ] ===================== --%>
			...
			<form action="BoardList" method="get">
				...
				<%-- 파라미터에 해당항목이 있을경우 해당내용 표시 --%>
				<select name="searchType">
					<option value="subject"<c:if test="${param.searchType eq 'subject' }">seleted</c:if>>제목</option>
					<option value="content"<c:if test="${param.searchType eq 'content' }">seleted</c:if>>내용</option>
					<option value="subject_content"<c:if test="${param.searchType eq 'subject_content' }">seleted</c:if>>제목&내용</option>
					<option value="name"<c:if test="${param.searchType eq 'name' }">seleted</c:if>>작성자</option>
				</select>
