			...
			<table border="1">
				...
				<tr>
					...
					<td>
						...
						<input type="text" name="id" id="id" placeholder="4 ~ 8글자 사이 입력" onblur="checkIdLength()" value="${member.id}" disabled>
				...
				<tr>
					<th>주소</th>
					<td>
						<input type="text" id="postcode" name="post_code" size="6" readonly placeholder="우편번호" value="${member.post_code}">
						<input type="button" value="주소검색" onclick="search_address()"><br>
						<input type="text" id="address1" name="address1" size="25" readonly placeholder="기본주소" value="${member.address1}"><br>
						<input type="text" id="address2" name="address2" size="25" placeholder="상세주소" value="${member.address2}">
					</td>
				</tr>
				<tr>
					<th>E-Mail</th>
					<td>
<!-- 					이메일 주소 분리(기준 문자열 @) -->
<!-- 						jstl-function 라이브러리의 split() 필요-->
						<%-- 1) 이매알 주소를 분리하여 변수에 저장 
								기본문법 : \${fn:split(원본문자열,기준문자열)}--%>
						<c:set var="arrEmail" value="${fn:split(member.email,'@')}"></c:set>
						<%-- 분리된 각 문자열을 저장 --%>
						<input type="text" size="10" id="email1" name="email1"  value="${arrEmail[0]}">@<input type="text" size="10" id="email2" name="email2" value="${arrEmail[1]}">
				...
				<tr>
					...
					<td>
						<select name="job">
						<%--member.job 값에따라 직업항목 셀렉트 박스의 option ...주석 추가 --%>
						<c:if test="${member.job eq '개발자'}">selected</c:if>
							<option value="">항목을 선택하세요</option>
							<option value="개발자" <c:if test="${member.job eq '개발자'}">selected</c:if>selected>개발자</option>
							<option value="DB엔지니어" <c:if test="${member.job eq 'DB엔지니어'}">selected</c:if>selected>DB엔지니어</option>
							<option value="관리자" <c:if test="${member.job eq '관리자'}">selected</c:if>selected>관리자</option>
						</select>
					</td>
				</tr>
				<tr>
					<th>성별</th>
					<td>
						<input type="radio" name="gender" value="남" <c:if test="${member.gender eq '남'}">checked</c:if>checked>남
						<input type="radio" name="gender" value="여" <c:if test="${member.gender eq '여'}">checked</c:if>checked>여
					</td>
				</tr>
				<tr>
					<th>취미</th>
					<td>
					<%-- member.hobby값에 따라서 해당취미 포함 여부를 판별하여 해당항목 체크(checked) 스플릿함수를 통해
						분리해서 판별하거나 contains()함수를 통해 분리없이 문자열 --%>
				...
				<tr>
					<th>프로필 이미지</th>
					<td>
						<img src="${pageContext.request.contextPath}/resources/images/profile_default.png" id="preview_profile" height="100px"><br>
						<input type="file" name="profile_img" id="profile_img">
					</td>
				</tr>
				<tr>
					<td colspan="2" align="center">
						<input type="submit" value="정보 수정" id="btnSubmit">
